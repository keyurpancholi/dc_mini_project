...
import threading
import socket
""" host name is retrieved using gethostbyname and a socket object is
created
"""
...
"""
s.listen(10)
"""Lists are created to store socket objects and processes ,when ever a
new process is connected to server,its socket object
is stored in process_sockets_list.process_list is used to store the id of
the process.
"""
...
neighbor_list = []
msg_token = ""
""" recv_messsage function gets socket object as input parameters and it
receives messages from the that process and sends it next process
along the ring."""
def recv_message(conn):
    while True:
        """ try receiving message from process and if it doesn't it throws
        the eception which makes it continue
        to receive message"""
        try:
        ...
        if "Coordinator: " in msg_token :
            le=msg_token.split()
            leader=le[1]
        """storing the index of the process id of the coordinator."""
        process_index = process_sockets_list.index(conn)
        """ ths is used to know to whom the message is to be redirect if
        process sending the message is last in list
        then the next process is first process, otherwise the next
        process.as the processes are communicating in the ring"""
        if len(process_sockets_list)==process_index+1 :
            to_process=0
        else :
            to_process=process_index+1
        """ if the server unable to send the message to next process that
        means the process is stopped
        this will throw an exception to remove the process from
        process_list and removes the socket object in
        the process_sockets_list and closing that socket connection"""
        try :
            process_sockets_list[to_process].send(received)
            """redirecting the received message to next process in the
            ring"""
            print("sending :" + received.decode('utf-8'))
            """ if unable to send the message that means that process is
            stopped ,so its socket object and
            process id's are remove from respective list."""
        except :
            """if the stopped process is not the coordinator, then the
            message is redirected towards to the next process in the ring"""
        ...
        while True:
            ...
            try:
                ...
                """appending the new connection's socket and storing it in
                process_sockets_list """
                ...
                """once server gets process id of the client server forks a new
                thread to client
